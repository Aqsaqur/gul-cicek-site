      ...
      <div className="md:col-span-2 bg-bgLight relative flex items-end justify-end rounded-lg overflow-hidden group">
        <div className="h-full z-10 absolute left-10 top-0 flex flex-col justify-center items-start gap-5 md:gap-10">
          <div className="flex flex-col gap-1 md:gap-3">
            <button className="bg-lightPink text-white rounded-full w-20 py-1 text-sm font-semibold hover:bg-green-600 hoverEffect">
        ...
        {singleBanner?.image && (
          <Image
            ...
            alt={banners[0].title}
            className="object-contain h-72 md:h-full max-h-[900px] self-end group-hover:scale-105 hoverEffect"
          />
        )}
      </div>

      {/* Right half - two stacked images */}
      <div className="flex flex-col space-y-5 md:space-y-10 h-auto md:max-h-[900px]">
          ...
          >
            <div className="w-1/2 flex flex-col">
              ...
              <p className="mt-3 font-medium text-black/60">
                ...
                <FormattedPrice
                  amount={item?.price}
                  className="text-lightPink font-bold"
                />
              </p>
              <Link
                href={"/shop"}
                className="mt-5 font-bold underline underline-offset-2 decoration-[1px] hover:text-lightPink hoverEffect"
              >
                Shop now!
              </Link>
            </div>
